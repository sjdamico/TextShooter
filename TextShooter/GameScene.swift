...
class GameScene: SKScene, SKPhysicsContactDelegate {          // Creates game scene with content
    private var levelNumber: Int    // Game level number property
    private var playerLives: Int {    // Number of player lives property
        didSet {
            let lives = childNodeWithName("LivesLabel") as! SKLabelNode
            lives.text = "Lives: \(playerLives)"
        }
    }
    private var finished = false    // Is level finished flag
    ...
    // Creates a level and setting level number at once
    class func scene(size:CGSize, levelNumber:Int) -> GameScene {
        return GameScene(size: size, levelNumber: levelNumber)
    }
    
    // Overides default initializer, passing control and a default level number to the designated initializer
    override convenience init(size:CGSize) {
        self.init(size: size, levelNumber: 1)
    }
    
    // Sets initial values of the level number and player lives
    // Common pattern when adding new initializers to a class while running the class's designated initializer
    init(size:CGSize, levelNumber:Int) {
        ...
        // Player lives instance node (displays players lives)
        ...
        // Game level instance node (displays game level)
        ...
        addChild(level)             // Adds level label to scene
        
        playerNode.position = CGPointMake(CGRectGetMidX(frame), CGRectGetHeight(frame) * 0.1)
        addChild(playerNode)        // Adds playerNode to scene
        
        addChild(enemies)
        // Adds enemies node to scene
        spawnEnemies()
        
        addChild(playerBullets)     // Adds bullets to the scene
    ...
    // All SpriteKit Nodes and SKScenes conform to NSCoding protocol
